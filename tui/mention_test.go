...
func TestMentions_BadgesTheReaderAndLeavesOthersPlain(t *testing.T) {
	me := mentionsIn(selfMention, "ou_me").render("@林岚 reachable")
	require.Equal(t, stMentionMe.Render("@林岚")+" reachable", me)

	other := mentionsIn(`[{"id":"ou_ma","key":"@_user_1","name":"秦风"}]`, "ou_me").render("@秦风 unreachable")
	require.Equal(t, "@秦风 unreachable", other, "someone else's mention is body text")

	require.Equal(t, "@林岚 hi", mentionsIn(selfMention, "").render("@林岚 hi"),
		"before the identity sync lands nothing is the reader's own")
}
...
func TestMentions_LongerNameWinsOverTheOneItContains(t *testing.T) {
	...
	require.Equal(t, stMentionMe.Render("@张三")+" 在吗", m.render("@张三 在吗"))
}

func TestMentions_OnDimsTheTextAround(t *testing.T) {
	out := mentionsIn(selfMention, "ou_me").on(stDim).render("群里 @林岚 看下")
	require.Equal(t, stDim.Render("群里 ")+stMentionMe.Render("@林岚")+stDim.Render(" 看下"), out)
}
...
func TestMentions_PostTagResolvesToAName(t *testing.T) {
	...
	require.Equal(t, stMentionMe.Render("@林岚")+" 在吗",
		mentionsIn("", "ou_me").render(`<at user_id="ou_me">林岚</at>在吗`))

	require.Equal(t, "@秦风 在吗",
		mentionsIn("", "ou_me").render(`<at user_id="ou_ma">秦风</at>在吗`))

	require.Equal(t, "@ou_x hi", mentionsIn("", "ou_me").render(`<at user_id="ou_x"></at>hi`),
