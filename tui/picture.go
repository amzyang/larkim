...
const (
	...
	// picMaxCols and picMaxRows keep one picture from taking the whole pane;
	// the message around it has to stay readable.
	picMaxCols = 40
	picMaxRows = 12
...
// because they were transmitted for the old one.
func (p *pictures) setCellSize(w, h int) {
...
// place is the size a file gets within maxCols columns, in cells. A zero size
// means there is nothing to draw: no graphics, no readable file, or no room.
func (p *pictures) place(path string, maxCols int) picture {
	if p == nil || path == "" || maxCols < 2 {
		return picture{}
	}
	abs := path
	...
	cw, ch := p.cell()
	// The terminal draws the picture into exactly the box it is given, so the
	// box is rounded to the nearest cell rather than up: half a cell of
	// rounding is half a cell of stretch.
	cols := min(maxCols, picMaxCols, max(1, (px.X+cw/2)/cw))
	rows := max(1, (cols*cw*px.Y/px.X+ch/2)/ch)
	if rows > picMaxRows {
		// Too tall at that width: let the height decide and take the columns
		// the aspect ratio then asks for.
		rows = picMaxRows
		cols = max(1, min(cols, (rows*ch*px.X/px.Y+cw/2)/cw))
	}
	return picture{path: abs, cols: cols, rows: rows}
}
...
func (p *pictures) prepare(pics []picture) string {
	...
	for _, pic := range pics {
		...
		cw, ch := p.cell()
		img, err := loadImage(pic.path, pic.cols*cw, pic.rows*ch)
		if err != nil {
			p.failed[pic.path] = true
			continue
		}
		id := p.take(key)
		if err := transmitPicture(&out, id, img, pic.cols, pic.rows); err != nil {
