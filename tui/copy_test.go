...
func TestOnVisualKey_EscCancelsAndYLeavesVisual(t *testing.T) {
	m := sized(120, 36)
...
func copyFixture(t *testing.T) Deps {
	...
	require.NoError(t, st.UpsertChats(ctx, []store.Chat{{ChatID: "oc_a", Name: "前端架构组", ChatMode: "group"}}, 1))
	require.NoError(t, st.UpsertContacts(ctx, []store.Contact{
		{OpenID: "ou_me", Name: "邹阳", Email: "zouyang@example.com"},
	...
	msgs := []store.Message{
		...
		{MessageID: "om_reply", ChatID: "oc_a", CreateMs: ms(2 * time.Hour), MessagePosition: -3, SenderID: "ou_me", SenderName: "邹阳", ThreadID: "omt_1", RawJSON: "{}"},
...
func TestCopyContext_RangeDropsThreadRepliesAndResolvesEveryone(t *testing.T) {
	...
	require.Equal(t, "前端架构组", got.chat)
	require.Contains(t, got.text, "## chat 前端架构组 oc_a group 3人 external:false")
	require.Contains(t, got.text, "邹阳 <zouyang@example.com> ou_me (me)")
...
func TestCopyContext_EmptyRangeStillCarriesTheHeader(t *testing.T) {
	...
	require.Contains(t, got.text, "## chat 前端架构组 oc_a group")
...
func TestUpdate_ContextMessageReportsSizeAndSetsTheClipboard(t *testing.T) {
	m := New(Deps{})
	mm, cmd := m.Update(contextMsg{text: strings.Repeat("x", 31*1024), n: 24, chat: "前端架构组"})
	require.Equal(t, "copied 24 msgs · 31 KB · 前端架构组", mm.(Model).notice)
	require.NotNil(t, cmd)

	mm, _ = m.Update(contextMsg{text: strings.Repeat("x", 205), n: 0, chat: "前端架构组"})
	require.Equal(t, "copied 0 msgs · 0.2 KB · 前端架构组", mm.(Model).notice)
...
func TestHelpText_DocumentsTheCopyKeys(t *testing.T) {
	...
	require.NotContains(t, helpText, "Y copy chat id", "Y is no longer bound")
}

func TestOnNormalKey_ShiftYIsUnbound(t *testing.T) {
	m := sized(120, 36)
	m.focus = paneMessages
	m = m.notify("", false)
	_, cmd := m.onNormalKey("Y")
	require.Nil(t, cmd)
}
