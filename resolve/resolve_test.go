...
func TestChat_LocalNameIgnoresSpacesThenRemote(t *testing.T) {
	...
	require.NoError(t, r.Store.UpsertChats(ctx, []store.Chat{{ChatID: "oc_1", Name: "FDEV兜底", ChatMode: "group"}}, 1))

	c, err := r.Chat(ctx, "FDEV 兜底")
	require.NoError(t, err)
...
func TestUser_EmailViaRemoteIsCached(t *testing.T) {
	r, f := newResolver(t)
	ctx := context.Background()
	f.Users = []larkcli.User{{OpenID: "ou_z", Name: "邹洋", Email: "zouyang@gaotu.cn", P2PChatID: "oc_p"}}

	c, err := r.User(ctx, "zouyang@gaotu.cn")
	require.NoError(t, err)
