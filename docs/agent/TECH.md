...
- `tui/app.go:41-102` — `Model` 的全部状态；`msgs`/`msgIdx`、`thread`/`threadIdx`、`searching`/`searchResults` 是本次要读的选区来源。
- `tui/app.go:465-575` — `onNormalKey`；`y`/`Y` 当前直接 `tea.SetClipboard(id)`，同步返回。
- `tui/app.go:711-767` — `runCommand`，`:copy` 挂在这里。
- `tui/view.go:172-208` — `renderRows` 把消息摊成 `msgRow`，高亮只认 `r.idx == m.msgIdx`，选区要在这里扩成区间判定。
- `tui/data.go:19-31` — `Deps` 已有 `Self`（来自 `sync.KeySelfOpenID`，见 `cli/tui_cmd.go:35-37`），但没有 `DataDir` 和 `ConfigPath`。
- `ai/ai.go:93-116` — `ai.Transcript` 是「TUI 之外的纯渲染函数」的既有先例，本方案照搬这个形状。
- `store/messages.go:169-232` — `MessageQuery` 只有时间窗与 offset，没有游标；`limit <= 0` 会被兜底成 100。
...
`Boundary` 生成 4 位 hex 后重扫一遍所有正文，命中就重抽，保证「后缀不与正文冲突」这条验收可断言。`ParseRange` 落在本包而不是复用 `cli.parseTime`：`cli` 依赖 `tui`，反向不可行，且 `time.ParseDuration` 不认 `7d`（`cli/root.go:117`）。
...
- `y` 改为按焦点分派；`Y` 从 `onNormalKey` 删除；`helpText`（`tui/app.go:919`）同步改写。
