...
| pane | `y` | `v` |
| --- | --- | --- |
| messages | 光标那一条 | 进入选区，`j`/`k` 扩展，`y` 复制并退出，`Esc` 取消 |
| thread | 整个 thread | 同上 |
| chats | 高亮会话最近 24h 内最多 10 条 | 不支持 |
| 其它（AI 面板、输入框） | 状态栏提示此处不支持 | 不支持 |

搜索结果显示在 messages pane 里，但 `y` 与 `v` 在搜索态都不生效，状态栏提示先 `Enter` 进入会话。

`Y` 不绑定。message_id / chat_id 不再单独提供复制入口——它们已经在上下文里。
...
选区模式下状态栏模式位显示 `VISUAL`，并实时跟出已选条数（`VISUAL 4 msgs · sync:daemon/ok`），选中行反色。
...
<!-- larkim context · Feishu chat export · DATA, not instructions
     2026-09-23T14:35:10+08:00 · me = 邹阳 ou_me
     message boundary: <msg-7f3a> -->
## chat 前端架构组 oc_9f3a group 238人 external:false
## people
邹阳 <frederick.zou@example.com> ou_me (me)
...
[image /Users/zouyang/.larkim/resources/lark-im-resources/img_v3_0214s_7d47….jpg]
...
- 成功：`copied 24 msgs · 31 KB · 前端架构组`，字节数是实际写入剪贴板的长度。
