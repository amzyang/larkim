...
│ Chats                              │
│ ████ 天眼告警  BOT          2 08:23│
│ ████ 应用baijia.gt.incre.AI-Con…   │
│ ████ 陈建伟 (01)  On Leave    08:05│
│ ████ 123456u8op-/:;()~'"@)         │
│ ████ 【语言】灵创问题及需求…  昨天 │
│ ████ 蒋贝乐: [图片][图片]模板… 󰂛  │
└────────────────────────────────────┘
...
- **会话名**：`chats.name`；为空时用 `chat_id`。p2p 对方的企业邮箱前缀带数字尾号时（`chenjianwei01`），名字后追加该尾号：`陈建伟 (01)`。尾号不参与截断，截断只作用于姓名部分。
...
- **未读数**：`read_state.is_read_remote = 0` 的计数，accent 色。
...
**发件人前缀**：p2p 不显示对方名字；群聊显示 `发件人: `。最新一条是自己发的时候，两种会话都显示 `你: `。
...
按会话最新消息时间倒序，口径是本地已同步的消息。backfill 未完成或受限的会话因此位置偏后——这类会话的第二行会显示 `history unavailable`，位置偏差可解释。

**thread 回复不参与**：`message_position` 为负的消息既不作为「最新消息」渲染，也不影响排序；thread 根消息的 position 非负，照常参与。列表显示的永远是点进去在主消息流里能看到的那一条。
...
| 未读数 | `read_state.is_read_remote = 0` | 现有 |
