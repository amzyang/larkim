...
TUI 左侧会话列表按飞书桌面端的信息密度重做：头像 + 两行文本，一眼能看出「哪些会话需要我处理」。列表里有两种行：会话，以及我参与过的 thread——客户端也这么列，thread 不顶起它所在的会话，而是自己占一行。

## 验收标准

扫一眼 larkim 的会话列表就能分活，不必切回飞书客户端确认。具体到可观察的判据：**@我、未读、草稿、发送失败**四类信号在首屏均可见且准确，我参与过的 thread 有新回复时自己成行。
...
列预算：pane 外宽 38，去边框 2 得 36 列内容区；头像 4 列 + 间距 1 列；文本区 31 列。每一行固定两行文本。

### 头部

`Chats<未读行数>` 左对齐，`<免打扰提示点>` 右对齐。头部是整份列表的概览，回答「还有多少行在等我」，不是「还有多少条消息」——每一行的消息数已经在各自行上。

- **未读行数**：带未读的行数，会话与 thread 同样计入，上标红色数字，免打扰的不计入；为 0 时不画。超过 99 画 `⁹⁹⁺`。上标把数字挂在 pane 名后面，读作一个词（`Chats³`），而不是两个字段。
...
- **未读数**：主消息流里飞书仍报未读、且在 larkim 里也没被读过的计数（`read_state.is_read_remote = 0` 且 `local_read_at = 0`），未读红；免打扰会话画成淡色。头像上画得出徽章时不重复画这个数字；画不出的会话（解码失败、非 kitty 终端）落回文字，两种画法同色。thread 回复不计进会话行——它们数在 thread 自己那一行上，见「thread 行」。
...
**thread 回复不参与会话行**：`message_position` 为负的消息既不计会话的未读、不作为会话的「最新消息」渲染，也不影响会话的位置；thread 根消息的 position 非负，照常参与。thread 存在的意义就是回复一个久远话题不必惊动整个会话，把会话顶回列表顶端正是它要避免的。会话行显示的永远是点进去在主消息流里能看到的那一条；回复归 thread 自己那一行说。

**免打扰只改变画法与概览口径**：muted 会话在列表里照常计未读，只是计数画成淡色（头像上的徽章走飞书自己的灰）。头部的概览不同——它把 muted 会话排除在计数之外，只用一个淡色点说明「静音的那些也有东西」。

## thread 行

我参与过的 thread 在列表里自己占一行，与会话行按时间交错。客户端就是这么列的：会话不被回复顶起来，读者仍看得见哪个话题动了。

**参与过**的口径与消息面板上的未读点一致：我在里面说过话（根消息算我的话），或者有人在里面 `@` 我。大群里与我无关的话题不进列表。

- **进出**：有回复才成行——没人回的根就是它所在会话的最新消息，会话行已经在说它了。读完仍在原处，只是徽标归零；位置由最后一条回复决定，与会话行同一条时间轴。
- **头像列**：客户端自己的话题标记（青色圆环加话题字形）托着该会话的头像，头像压在环的右下角、四周留一圈空隙，与客户端列表里的那一个同形。画不出图的终端（非 kitty）落回文字：`⤷` 占一列，余下是会话头像的色块，与消息面板上话题摘要行的记号同字。
- **第一行**：`<root 发件人>: <root 摘要>` 左对齐，`<未读回复数> <最后回复时间>` 右对齐。标题承担截断。
- **第二行**：`<@我><最后一条回复的发件人>: <摘要>`，前缀规则与会话行相同（自己 `You: `、p2p 不写名字）。`@` 槽在未读回复里有人叫我时亮。撤回的最后一条写 `<发件人> recalled a message`，行不动。
- **静音**：继承所在会话——静音的回复不进这一行的时间与计数，计数画成淡色。
- **打开**：光标停住、`Enter`、单击开的是同一样东西——所在会话的消息页，外加右栏里的这个 thread，栏内光标落在最后一条回复上。随后由话题面板照常结算已读，徽标归零而行仍在。
- **焦点**：光标停住、单击、`n` / `N`、过滤后回车都把焦点留在会话列表——读者正在读这张列表，这一行是被看的，不是被去的；只有 `Enter` 把焦点交给右栏，它与会话行的 `Enter` 交给消息栏同义。从右栏回来走 `Esc`，在两栏之间走 `Tab` / `h` / `l`。
- **`n` / `N`**：与会话行同列，thread 行也在队列里；它带的是一个数字，与会话行同类。
...
- 数据层与排版走白盒测试：摘要拼接、时间戳分档、截断优先级、槽位令牌、`你: ` 前缀规则、四种空态文案、thread 回复不进会话行、thread 行的两行版式与交错顺序、撤回渲染、账号尾号的提取与免截断。
...
- 过滤匹配会话行的发件人或消息摘要。thread 行例外：它的标题就是 root 的正文，那是这一行的名字。
