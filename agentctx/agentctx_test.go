...
func group() Input {
	return Input{
		...
		Boundary: "7f3a",
		Self:     Person{Name: "林岚", OpenID: "ou_me", Email: "linlan@example.com"},
		Chat:     store.Chat{ChatID: "oc_9f3a", Name: "平台组", ChatMode: "group"},
		Members:  238,
		People: []Person{
			{Name: "林岚", OpenID: "ou_me", Email: "linlan@example.com"},
...
func TestRender_HeaderNamesChatPeopleAndBoundary(t *testing.T) {
	out := Render(group())
	require.Contains(t, out, "2026-09-23T14:35:10+08:00 · me = 林岚 ou_me")
	require.Contains(t, out, "message boundary: <msg-7f3a> -->")
	require.Contains(t, out, "## chat 平台组 oc_9f3a group 238人 external:false\n")
	require.Contains(t, out, "林岚 <linlan@example.com> ou_me (me)\n")
...
func TestRender_APIUpdatedFlagAloneIsNotAnEdit(t *testing.T) {
	...
	in.Messages = []store.Message{
		{MessageID: "om_a", CreateMs: at(10, 0), SenderName: "张三", SenderID: "ou_a", Updated: true, Content: "x", RenderedAt: 5},
	}
	require.NotContains(t, Render(in), "edited")
}
...
func TestRender_AttributeMatrix(t *testing.T) {
	...
	in.Messages = []store.Message{
		{MessageID: "om_a", CreateMs: at(10, 0), SenderName: "张三", SenderID: "ou_a", ThreadID: "omt_1", ReplyTo: "om_z",
			EditedAt: 9, Content: "x", RenderedAt: 5, MentionsJSON: `[{"key":"@_user_1","id":"ou_b","name":"李四"},{"key":"@_user_2","id":"ou_c","name":"王五"}]`,
